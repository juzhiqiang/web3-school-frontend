...
import AaveStaking from "./components/AaveStaking";

function TokenSwap() {
  const { isConnected, balance, refetchBalance } = useWeb3();
  const {
    contractAddress,
    networkName,
    isLocalNetwork,
  ...
  const [uniswapMode, setUniswapMode] = useState<"eth-to-usdt" | "usdt-to-eth">(
    "eth-to-usdt"
  );
  const [inputAmount, setInputAmount] = useState("");
  const [slippage, setSlippage] = useState<number>(
    TOKEN_SWAP_CONFIG.DEFAULT_SLIPPAGE
  );
  ...
  const toggleSwapMode = () => {
    if (swapType === "yideng") {
      ...
      setUniswapMode((prev) =>
        prev === "eth-to-usdt" ? "usdt-to-eth" : "eth-to-usdt"
      );
    }
    setInputAmount("");
  };
  ...
  const getButtonConfig = () => {
    const currentIsLoading =
      swapType === "yideng" ? isLoading : isUniswapLoading;
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4">代币兑换 & 理财</h2>
          <p className="text-gray-600">请先连接您的钱包以开始使用。</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6">
          <div className="flex items-center justify-center space-x-3 mb-2">
            <h1 className="text-3xl font-bold">代币兑换 & 理财</h1>
            <button
          ...
          <p className="text-gray-600 mb-6">安全、快速的代币兑换和理财服务</p>
        </div>
        ...
        {/* AAVE理财组件 */}
        <AaveStaking onRefresh={handleGlobalRefresh} />
            ...
            <div className="flex items-start space-x-2">
              ...
              <div>
                ...
                <p className="text-sm text-red-700">
                  ...
                  {isLocalNetwork &&
                    "请确保在Ganache网络中部署了合约，并设置正确的合约地址。"}
                </p>
                {isLocalNetwork && (
